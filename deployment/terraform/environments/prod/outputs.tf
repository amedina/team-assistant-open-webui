...
output "application_url" {
  ...
  value       = var.custom_domain != "" ? "https://${var.custom_domain}" : module.cloud_run.service_url
}

output "oauth_redirect_uri" {
  description = "OAuth redirect URI to configure in Google Cloud Console"
  value       = var.custom_domain != "" ? "https://${var.custom_domain}/oauth/google/callback" : "${module.cloud_run.service_url}/oauth/google/callback"
}
...
output "deployment_summary" {
  ...
  value = {
    environment         = "prod"
    application_url     = var.custom_domain != "" ? "https://${var.custom_domain}" : module.cloud_run.service_url
    cloud_run_url      = module.cloud_run.service_url
    custom_domain      = var.custom_domain != "" ? var.custom_domain : "Not configured"
    database_tier       = var.database_tier
    database_ha        = var.enable_high_availability
